...
bool MyScene::create( std::istream & inputfile, bool active)
{
  using namespace std;
  ...
  for (int i = 0; i < y; i++)			// down (row)
  {
    for (int j = 0; j < x; j++)		// across (column)
    {
      ...
      if (rent != NULL)		// it might not be an agent or object
      {
        ...
        else if(c=='k') agent = new MyChickenAgent(true);
        else if(c=='d') agent = new MyDragonAgent(true);
		else if (c == 'b') agent = new MyBallAgent(true);
		else if (c == 't') agent = new MyTowerAgent(true);
		...
		agent->startX = j*cell_w;
		agent->startY = i * cell_h;
...
bool MySceneManager::create(std::string name, std::string scene_file, bool active)
{
...
//   inputfile.open(path);
  ...
  assert(level);
  if(!level->create(inputfile, active)) return false;
  this->add(name,level);
  return true;
}
